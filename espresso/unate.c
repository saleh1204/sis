	...
	pset_family A; /* disposes of A */
{
	 ...
	 */
		...
		foreachi_set(A, i, p)
		{
			...
			{
				set_copy(restrict_reg, p);
				min_set_ord = SIZE(p);
			}
			else if (SIZE(p) == min_set_ord)
			{
				set_or(restrict_reg, restrict_reg, p);
			}
		}
		...
		{
			Abar = unate_complement(abs_covered_many(A, restrict_reg));
			sf_free(A);
			foreachi_set(Abar, i, p)
			{
				set_or(p, p, restrict_reg);
			}
		...
		{
			max_i = abs_select_restricted(A, restrict_reg);
 ...
 */
static int
	abs_select_restricted(A, restrict_reg)
		pset_family A;
pset restrict_reg;
{
