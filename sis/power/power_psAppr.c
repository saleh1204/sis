...
int nPSLines;
{
    ...
    result = power_count_f_cof_probPS(f_bdd, psProb, nNSLines, nPSLines, (long)-1, sets);
...
pset sets;
{
    ...
    if (topf >= nPSLines)
    { /* First PI found, all PIs from now on */
...
static void multiply_PS_prob(result, index, psProb, sets, nNSLines) double *result;
long index;
double *psProb;
pset sets;
int nNSLines;
{
    ...
    if (power_setSize == 1)
    { /* This if is not need, but is much simpler */
        if (is_in_set(sets, 1))
...
static void calculate_next_iteration(allpsLinesProb, psLinesProb, Y, J, nNSLines) double *allpsLinesProb;
double **psLinesProb;
double **Y;
double *J;
int nNSLines;
